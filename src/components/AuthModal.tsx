...
import { auth, buildDefaultUser, getUserProfile, saveUserProfile, formatFirebaseAuthError, AVATAR_PRESETS } from '../lib/firebase';
import { supabaseResetPassword } from '../services/supabaseService';
...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMessage('Compte créé avec succès ! Bienvenue sur AfriChat Connect.');
      setTimeout(() => {
      ...
      >
        ...
        <div className="p-5 sm:p-6 overflow-y-auto space-y-4">
          ...
          {mode === 'signup' && (
            <form onSubmit={handleSignUp} className="space-y-4">
              ...
              <div className="p-3 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex items-center space-x-2 text-xs text-amber-300">
                <Sparkles className="w-4 h-4 text-amber-400 shrink-0" />
                <span>Bonus de bienvenue : <strong>15 000 FCFA</strong> crédités pour vos premiers dons & achats !</span>
              </div>
              ...
              >
                {isLoading ? (
                  <>
                    <Loader2 className="w-4 h-4 animate-spin" />
                    <span>Création de votre profil Firebase...</span>
                  </>
                ) : (
                  <>
        ...
        <div className="p-3 bg-stone-950 border-t border-stone-800 flex items-center justify-between text-[11px] text-stone-400 px-4 sm:px-6">
          <div className="flex items-center space-x-1.5">
            ...
            <span>Données protégées par Firebase Auth & Cloud Firestore</span>
          </div>
          <span className="font-mono text-[10px] text-amber-400">v2.4-auth</span>
        </div>
      </motion.div>
    </div>
  );
